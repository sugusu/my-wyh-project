...
mkdir -p /data/wyh/DeformTransGS/baselines/tsgs_synth_canonical_asg

python3 train.py \
    -s /data/wyh/DeformTransGS/data/stage2_canonical_tsgs/dataset_full12 \
    -m /data/wyh/DeformTransGS/baselines/tsgs_synth_canonical_asg \
    ...
    --data_device cuda \
    --use_asg \
    2>&1 | tee /data/wyh/DeformTransGS/baselines/tsgs_synth_canonical_asg/train.log

echo "Model A training exit: $?"
